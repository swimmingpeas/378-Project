...
With this development there comes another point of growth: feature selection and aggregation. As a byproduct of us using a smaller sample size when training our current models, we were forced to aggregate the data in a format that did not create too many parameters for our model, as to avoid overfiting. However, if we implement the ideas above, we no longer face that issue and can broaden over feature possibilities once again. Another potential direction is to not aggregate the features at all, and instead include complete information of a smaller subset of them which we beleive to be the most important. For instance, just the spectrogram of the audio clip. Through doing this, we do not lose any information which may be lost through simply looking at the mean, standard deviation, and extrema of the features. Additionally, there is a chance that minimizing the number of features we include could prevent the models from being confused by confounding attributes. For instance, there may be very high correlation between the mean of the MFCCs and the standard deviation of the spectral centroid, but we can't really know that for sure until we see the efficacy of our models on different subsets of feature information. Thus, attempting to build CNNs with fewer, full feature information is another potential direction for improvement.
...WRITE MORE HERE
